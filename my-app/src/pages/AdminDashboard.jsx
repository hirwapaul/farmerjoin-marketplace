...
function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const cooperativesResponse = await API.get("/cooperative/admin/cooperatives", {
        headers: { Authorization: `Bearer ${token}` }
      });
      setCooperatives(cooperativesResponse.data);
  ...
  const handleEditUser = (user, type) => {
    ...
    setCreateForm({
      ...
      email: user.email,
      phone: user.phone,
      cooperative_name: user.farm_name || user.cooperative_name,
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      if (createForm.role === "farmer") {
        response = await API.put(`/farmers/admin/${selectedUser.farmer_id}`, createForm, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else if (createForm.role === "cooperative") {
        response = await API.put(`/cooperative/admin/${selectedUser.cooperative_id}`, createForm, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
      
      alert(`${createForm.role.charAt(0).toUpperCase() + createForm.role.slice(1)} updated successfully`);
          ...
          {cooperatives.length > 0 && (
            <div className="bg-white shadow rounded-lg">
              <div className="px-4 py-5 border-b border-gray-200 sm:px-6">
                <h3 className="text-lg leading-6 font-medium text-gray-900">Cooperatives</h3>
                <p className="mt-1 text-sm text-gray-500">Manage cooperative accounts</p>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {cooperatives.map((cooperative) => (
                      <tr key={cooperative.cooperative_id}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {cooperative.location || 'Not specified'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() => handleEditUser(cooperative, 'cooperative')}
                            className="text-indigo-600 hover:text-indigo-900 mr-3"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => handleDeleteUser(cooperative, 'cooperative')}
      ...
      {showManageModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
            <div className="mt-3">
              <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                {isEditing ? 'Edit' : 'Manage'} {createForm.role === "cooperative" ? "Cooperative" : "Farmer"} Account
              </h3>
              ...
              <form onSubmit={handleUpdateUser} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {createForm.role === "cooperative" ? "Cooperative Name" : "Farm Name"}
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    onChange={(e) => setCreateForm({...createForm, cooperative_name: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                    placeholder={createForm.role === "cooperative" ? "Cooperative Name" : "Farm Name"}
                  />
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors"
                  >
                    {creating ? "Updating..." : "Update Account"}
                  </button>
                </div>
              </form>
            </div>
